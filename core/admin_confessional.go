...
const confessionalLevelProfilePrefix = "/confessional/nivells/"
...
func (a *App) AdminSaveConfessional(w http.ResponseWriter, r *http.Request) {
	...
	if kind == "entitat" && data.Entitat != nil && data.PrimaryMunicipiID > 0 {
		if id == 0 {
	...
	if _, err := a.saveConfessionalData(kind, data); err != nil {
...
func (a *App) AdminConfessionalEntityShow(w http.ResponseWriter, r *http.Request) {
	...
	entityLabels := entitatReligiosaLabels(allEntitats)
	municipiLabelsMap := municipiLabels(municipis)
	...
	for _, rel := range relsSuperiors {
		parentLinks = append(parentLinks, confessionalProfileLink{
			Label: confessionalLabelWithFallback(entityLabels, rel.EntitatOrigenID),
	...
	for _, rel := range relsInferiors {
		childLinks = append(childLinks, confessionalProfileLink{
			Label: confessionalLabelWithFallback(entityLabels, rel.EntitatDestiID),
	...
	for _, rel := range relsTerritori {
		row := confessionalEntityTerritoryRelationView{
			...
			MunicipiLabel: confessionalLabelWithFallback(municipiLabelsMap, rel.MunicipiID),
		...
		if rel.NucliID.Valid {
			...
			row.NucliLabel = confessionalLabelWithFallback(municipiLabelsMap, row.NucliID)
	...
	levelProfileURL := confessionalLevelProfileURL(entitat.NivellConfessionalCodi)
	RenderPrivateTemplate(w, r, "admin-confessional-entity-show.html", map[string]interface{}{
		"Entitat":                         entitat,
		"EntitatLabels":                   entityLabels,
		"ReligionCatalogLabels":           confessionalReligionCatalogLabels(lang),
		"LevelCatalogLabels":              confessionalLevelCatalogLabels(lang),
		"RelationTypeLabels":              relationTypeLabels,
		"PaisLabels":                      paisLabels(paisos),
		"MunicipiLabels":                  municipiLabelsMap,
		...
		"LevelProfileURL":                 levelProfileURL,
...
func (a *App) AdminConfessionalLevelShow(w http.ResponseWriter, r *http.Request) {
	...
	parentLevels := confessionalParentLevelLinks(level, lang)
	childLevels := confessionalChildLevelLinks(level, lang)
	allEntities, _ := a.DB.ListEntitatsReligioses()
	entities := confessionalLevelEntityLinks(allEntities, level, religion, lang)
...
func confessionalParentLevelLinks(level ConfessionalLevelCatalogItem, lang string) []confessionalProfileLink {
	links := []confessionalProfileLink{}
	for _, parentCode := range level.AllowedParentLevelCodes {
		parent, ok := GetConfessionalLevelCatalogByCode(parentCode)
		if !ok || !parent.Active {
			continue
		}
		links = append(links, confessionalProfileLink{
			Label: ConfessionalLevelLabel(parent, lang),
			URL:   confessionalLevelProfileURL(parent.Code),
			Meta:  parent.Code,
		})
	}
	return links
...
func confessionalChildLevelLinks(level ConfessionalLevelCatalogItem, lang string) []confessionalProfileLink {
	links := []confessionalProfileLink{}
	for _, item := range ListSelectableConfessionalLevelCatalog() {
		if item.Code == level.Code || !item.Active || !ConfessionalParentLevelCompatible(level.Code, item.Code) {
			continue
		}
		links = append(links, confessionalProfileLink{
			Label: ConfessionalLevelLabel(item, lang),
			URL:   confessionalLevelProfileURL(item.Code),
			Meta:  item.Code,
		})
	}
...
func confessionalLevelEntityLinks(allEntities []db.EntitatReligiosa, level ConfessionalLevelCatalogItem, religion ConfessionalReligionCatalogItem, lang string) []confessionalProfileLink {
	links := []confessionalProfileLink{}
...
func (a *App) parseConfessionalForm(kind string, id int, r *http.Request, lang string) (confessionalFormData, string) {
	...
	case "entitat":
		primaryMunicipiID := parsePositiveIntDefault(r.FormValue("municipi_principal_id"), 0, 0, 1000000000)
		...
		data.Entitat = item
		data.PrimaryMunicipiID = primaryMunicipiID
		data.PrimaryMunicipiLabel = strings.TrimSpace(r.FormValue("municipi_principal_label"))
		data.InitialRelationNotes = strings.TrimSpace(r.FormValue("initial_relation_notes"))
		if item.Nom == "" {
			return data, "El nom es obligatori."
		}
		if item.Codi == "" {
		...
		if data.PrimaryMunicipiID > 0 {
			...
			if !ok || !level.CanLinkMunicipi {
				return data, T(lang, "confessional.initial_relations.primary_municipality.unsupported")
			}
...
func (a *App) saveConfessionalEntityWithInitialRelations(data confessionalFormData, userID int, needsParentRelation, needsPrimaryMunicipiRelation bool) (int, error) {
	if data.Entitat == nil {
		return 0, errors.New("entitat religiosa buida")
	}
	plan := &db.EntitatReligiosaInitialRelationsTxPlan{Entitat: data.Entitat}
	if needsParentRelation {
		plan.ParentRelation = a.buildConfessionalParentRelation(data.Entitat, 0, userID, data.InitialRelationNotes)
	}
	if needsPrimaryMunicipiRelation {
		plan.TerritoryRelation = a.buildConfessionalPrimaryMunicipiRelation(data.Entitat, 0, userID, data.PrimaryMunicipiID, data.InitialRelationNotes)
	}
	result, err := a.DB.SaveEntitatReligiosaWithInitialRelationsTx(plan)
	if err != nil {
		return 0, err
	}
	return result.EntitatID, nil
}

func (a *App) createConfessionalEntityInitialRelations(data confessionalFormData, entityID, userID int, needsParentRelation, needsPrimaryMunicipiRelation bool) error {
	if data.Entitat == nil {
		return nil
	}
	if needsParentRelation {
		if err := a.createConfessionalParentRelation(data.Entitat, entityID, userID, data.InitialRelationNotes); err != nil {
			return err
		}
	}
	if needsPrimaryMunicipiRelation {
...
func (a *App) createConfessionalParentRelation(entitat *db.EntitatReligiosa, childID, userID int, notes string) error {
	...
	_, err = a.DB.SaveEntitatReligiosaRelacio(a.buildConfessionalParentRelation(entitat, childID, userID, notes))
	return err
}
...
func (a *App) needsConfessionalPrimaryMunicipiRelation(entityID, municipiID int) (bool, error) {
	...
	for _, rel := range rels {
		...
		if rel.MunicipiID == municipiID && !rel.NucliID.Valid {
			return false, nil
		}
	}
	return true, nil
...
func (a *App) confessionalPrimaryMunicipiForEntity(entityID int) *db.MunicipiEntitatReligiosa {
	...
	return findConfessionalPrimaryMunicipiRelation(entityID, rels)
}
...
func (a *App) buildConfessionalParentRelation(entitat *db.EntitatReligiosa, childID, userID int, notes string) *db.EntitatReligiosaRelacio {
	if entitat == nil || !entitat.ParentID.Valid {
		return nil
	}
...
func (a *App) buildConfessionalPrimaryMunicipiRelation(entitat *db.EntitatReligiosa, entityID, userID, municipiID int, notes string) *db.MunicipiEntitatReligiosa {
	if entitat == nil || municipiID == 0 {
		return nil
	}
	return &db.MunicipiEntitatReligiosa{
		MunicipiID:         municipiID,
...
func confessionalLevelCodeFromPath(path string) string {
	if !strings.HasPrefix(strings.TrimSpace(path), confessionalLevelProfilePrefix) {
		return ""
	}
	return normalizeCatalogCode(strings.TrimPrefix(strings.TrimSpace(path), confessionalLevelProfilePrefix))
}
...
func confessionalLabelWithFallback(labels map[int]string, id int) string {
...
func confessionalLevelProfileURL(code string) string {
	code = normalizeCatalogCode(code)
	if code == "" {
		return ""
	}
	return confessionalLevelProfilePrefix + code
}
