...
func (a *App) LlibreWikiHistory(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.NotFound(w, r)
		return
	}
	if !isValidWikiObjectType("llibre") {
		http.NotFound(w, r)
		return
	}
	lang := ResolveLang(r)
	...
	changes, _ := a.DB.ListWikiChanges("llibre", llibreID)
	...
	if viewToken != "" {
		...
		case "current", "published":
			if payload, err := json.Marshal(llibre); err == nil {
	...
	titleLabel := strings.TrimSpace(llibre.Titol)
	if titleLabel == "" {
		titleLabel = llibre.NomEsglesia
	}
	title := fmt.Sprintf("%s: %s", T(lang, "books.title"), titleLabel)
	historyURL := fmt.Sprintf("/documentals/llibres/%d/historial", llibre.ID)
	RenderPrivateTemplate(w, r, "wiki-history.html", map[string]interface{}{
		"Title":             title,
		"BackURL":           fmt.Sprintf("/documentals/llibres/%d", llibre.ID),
		"HistoryURL":        historyURL,
		"RevertURL":         fmt.Sprintf("/documentals/llibres/%d/historial/revert", llibre.ID),
		"History":           history,
...
func (a *App) LlibreWikiStats(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.NotFound(w, r)
		return
	}
	...
	counts, _ := a.DB.GetWikiPublicCounts("llibre", llibreID)
...
func (a *App) LlibreWikiRevert(w http.ResponseWriter, r *http.Request) {
	...
	if change.ObjectType != "llibre" || change.ObjectID != llibreID {
		http.Error(w, "Canvi invàlid", http.StatusBadRequest)
		return
	}
	...
	if newID > 0 {
		detail := "llibre:" + strconv.Itoa(llibreID)
		_, _ = a.RegisterUserActivity(r.Context(), user.ID, ruleLlibreUpdate, "editar", "llibre_canvi", &newID, "pendent", nil, detail)
	}
	returnURL := strings.TrimSpace(r.FormValue("return_to"))
	if returnURL == "" {
		returnURL = fmt.Sprintf("/documentals/llibres/%d/historial", llibreID)
	}
	http.Redirect(w, r, returnURL, http.StatusSeeOther)
}

func (a *App) LlibreWikiMark(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.NotFound(w, r)
		return
	}
	...
	perms := a.getPermissionsForUser(user.ID)
	if llibre.ModeracioEstat != "publicat" && !(a.CanManageArxius(user) || a.hasPerm(perms, permModerate)) {
		http.NotFound(w, r)
		return
	}
	...
	prevMark, _ := a.DB.GetWikiMark("llibre", llibreID, user.ID)
	...
	if err := a.updateWikiPublicCounts("llibre", llibreID, prevMark, tipus, isPublic); err != nil {
...
func (a *App) LlibreWikiUnmark(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.NotFound(w, r)
		return
	}
	...
	prevMark, _ := a.DB.GetWikiMark("llibre", llibreID, user.ID)
	if err := a.DB.DeleteWikiMark("llibre", llibreID, user.ID); err != nil {
		Errorf("Error eliminant marca wiki: %v", err)
		http.Error(w, "No s'ha pogut eliminar la marca", http.StatusInternalServerError)
		return
	}
	if prevMark != nil {
		if err := a.updateWikiPublicCounts("llibre", llibreID, prevMark, prevMark.Tipus, false); err != nil {
			Errorf("Error actualitzant stats wiki: %v", err)
			http.Error(w, "No s'ha pogut eliminar la marca", http.StatusInternalServerError)
			return
		}
