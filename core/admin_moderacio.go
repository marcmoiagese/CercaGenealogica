...
type moderacioBulkRegistreChunkMetrics struct {
	...
	DerivedStatsPrepareSetupDur          time.Duration
	DerivedStatsPrepareRoleDur           time.Duration
	DerivedStatsPrepareNomDur            time.Duration
	DerivedStatsPrepareCognomDur         time.Duration
	DerivedStatsPrepareNivellsDur        time.Duration
	DerivedStatsPreparePersones          int
	DerivedStatsPrepareMatched           int
	DerivedStatsPrepareNomValues         int
	DerivedStatsPrepareCognomValues      int
	DerivedStatsPrepareNomCacheHits      int
	DerivedStatsPrepareNomCacheMisses    int
	DerivedStatsPrepareCognomCacheHits   int
	DerivedStatsPrepareCognomCacheMisses int
...
type moderacioBulkRegistreDerivedMetrics struct {
	...
	StatsPrepareSetupDur          time.Duration
	StatsPrepareRoleDur           time.Duration
	StatsPrepareNomDur            time.Duration
	StatsPrepareCognomDur         time.Duration
	StatsPrepareNivellsDur        time.Duration
	StatsPreparePersones          int
	StatsPrepareMatched           int
	StatsPrepareNomValues         int
	StatsPrepareCognomValues      int
	StatsPrepareNomCacheHits      int
	StatsPrepareNomCacheMisses    int
	StatsPrepareCognomCacheHits   int
	StatsPrepareCognomCacheMisses int
...
func newModeracioBulkRegistreSearchContext() *moderacioBulkRegistreSearchContext {
	return &moderacioBulkRegistreSearchContext{
		CognomCanonCache: map[string]string{},
		NormalizeCache:   map[string]string{},
		PhoneticCache:    map[string]string{},
		SearchDocCache:   map[int]db.SearchDoc{},
...
func measuredDurationAtLeast(elapsed time.Duration, parts ...time.Duration) time.Duration {
	totalParts := time.Duration(0)
	for _, part := range parts {
		totalParts += part
	}
	if elapsed < totalParts {
		return totalParts
	}
	return elapsed
}
...
func (a *App) applyModeracioBulkRegistreUpdates(action string, ids []int, motiu string, moderatorID int, metrics *moderacioApplyMetrics, onChunk func(moderacioBulkRegistreChunkMetrics)) moderacioBulkRegistreUpdateResult {
	...
	for start := 0; start < len(ids); start += chunkSize {
		...
		chunkMetrics.DerivedStatsPrepareSetupDur = derivedMetrics.StatsPrepareSetupDur
		chunkMetrics.DerivedStatsPrepareRoleDur = derivedMetrics.StatsPrepareRoleDur
		chunkMetrics.DerivedStatsPrepareNomDur = derivedMetrics.StatsPrepareNomDur
		chunkMetrics.DerivedStatsPrepareCognomDur = derivedMetrics.StatsPrepareCognomDur
		chunkMetrics.DerivedStatsPrepareNivellsDur = derivedMetrics.StatsPrepareNivellsDur
		chunkMetrics.DerivedStatsPreparePersones = derivedMetrics.StatsPreparePersones
		chunkMetrics.DerivedStatsPrepareMatched = derivedMetrics.StatsPrepareMatched
		chunkMetrics.DerivedStatsPrepareNomValues = derivedMetrics.StatsPrepareNomValues
		chunkMetrics.DerivedStatsPrepareCognomValues = derivedMetrics.StatsPrepareCognomValues
		chunkMetrics.DerivedStatsPrepareNomCacheHits = derivedMetrics.StatsPrepareNomCacheHits
		chunkMetrics.DerivedStatsPrepareNomCacheMisses = derivedMetrics.StatsPrepareNomCacheMisses
		chunkMetrics.DerivedStatsPrepareCognomCacheHits = derivedMetrics.StatsPrepareCognomCacheHits
		chunkMetrics.DerivedStatsPrepareCognomCacheMisses = derivedMetrics.StatsPrepareCognomCacheMisses
		...
		chunkMetrics.DerivedStatsApplyDur = derivedMetrics.StatsApplyDur
		chunkMetrics.DerivedStatsItems = derivedMetrics.StatsItems
		chunkMetrics.DerivedStatsNomKeys = derivedMetrics.StatsNomKeys
		chunkMetrics.DerivedStatsCognomKeys = derivedMetrics.StatsCognomKeys
		chunkMetrics.DerivedStatsDeltaRows = derivedMetrics.StatsDeltaRows
		chunkMetrics.DerivedStatsMunicipis = derivedMetrics.StatsMunicipis
		chunkMetrics.DerivedStatsNivells = derivedMetrics.StatsNivells
		chunkMetrics.DerivedStatsNegativeRows = derivedMetrics.StatsNegativeRows
		chunkMetrics.DerivedSearchDur = derivedMetrics.SearchDur
		chunkMetrics.SearchPrepareDur = derivedMetrics.SearchPrepareDur
		chunkMetrics.SearchBuildDur = derivedMetrics.SearchBuildDur
		chunkMetrics.SearchUpsertDur = derivedMetrics.SearchUpsertDur
		chunkMetrics.SearchDeleteDur = derivedMetrics.SearchDeleteDur
		chunkMetrics.SearchDocsUpserts = derivedMetrics.SearchDocsUpserts
		chunkMetrics.SearchDocsDeletes = derivedMetrics.SearchDocsDeletes
		chunkMetrics.SearchCacheHits = derivedMetrics.SearchCacheHits + warmupStats.Hits
		chunkMetrics.SearchCacheMisses = derivedMetrics.SearchCacheMisses + warmupStats.Misses
		chunkMetrics.SearchCacheSize = derivedMetrics.SearchCacheSize
		chunkMetrics.SearchDocCacheHits = derivedMetrics.SearchDocCacheHits
		chunkMetrics.SearchDocCacheMisses = derivedMetrics.SearchDocCacheMisses
		chunkMetrics.SearchDocCacheSize = derivedMetrics.SearchDocCacheSize
		chunkMetrics.DerivedDur = time.Since(derivedStart)
		chunkMetrics.PostprocDur = chunkMetrics.SearchWarmupDur + chunkMetrics.DerivedDur
...
func (a *App) warmModeracioBulkRegistreSearchCache(states []moderacioBulkRegistreState, successSet map[int]struct{}, estat string, searchCtx *moderacioBulkRegistreSearchContext) moderacioBulkRegistreSearchWarmupStats {
	...
	searchCtx.SearchDocCache = make(map[int]db.SearchDoc, len(successSet))
...
func (a *App) applyModeracioBulkRegistreDerivedSideEffects(states []moderacioBulkRegistreState, successSet map[int]struct{}, estat string, demoGroups map[moderacioBulkRegistreDemoKey][]int, nivellCache map[int][]int, searchCtx *moderacioBulkRegistreSearchContext) moderacioBulkRegistreDerivedMetrics {
	...
	statsStart := time.Now()
	statsPrepareStart := time.Now()
	statsItems := make([]nomCognomBulkDelta, 0, len(successByID))
	contribCache := newNomCognomContribCache()
	for _, state := range states {
		if _, ok := successSet[state.Reg.ID]; !ok {
			continue
		}
		...
		contribMetrics := nomCognomContribMetrics{Cache: contribCache}
		contribStart := time.Now()
		contrib := calcNomCognomContribsWithMetrics(state.Reg, state.Persones, &contribMetrics)
		metrics.StatsPrepareContribDur += time.Since(contribStart)
		metrics.StatsPrepareSetupDur += contribMetrics.SetupDur
		metrics.StatsPrepareRoleDur += contribMetrics.RoleMatchDur
		metrics.StatsPrepareNomDur += contribMetrics.NomDur
		metrics.StatsPrepareCognomDur += contribMetrics.CognomDur
		metrics.StatsPreparePersones += contribMetrics.Persones
		metrics.StatsPrepareMatched += contribMetrics.Matched
		metrics.StatsPrepareNomValues += contribMetrics.NomValues
		metrics.StatsPrepareCognomValues += contribMetrics.CognomValues
		metrics.StatsPrepareNomCacheHits += contribMetrics.NomCacheHits
		metrics.StatsPrepareNomCacheMisses += contribMetrics.NomCacheMisses
		metrics.StatsPrepareCognomCacheHits += contribMetrics.CognomCacheHits
		metrics.StatsPrepareCognomCacheMisses += contribMetrics.CognomCacheMisses
	...
	metrics.StatsPrepareDur = time.Since(statsPrepareStart)
	statsApplyMetrics, err := a.applyNomCognomBulkDeltas(statsItems)
	if err != nil {
		Errorf("Error actualitzant stats noms/cognoms bulk: %v", err)
	}
	metrics.StatsAggregateDur = statsApplyMetrics.AggregateDur
	metrics.StatsEnsureDur = statsApplyMetrics.EnsureDur
	metrics.StatsBuildDeltasDur = statsApplyMetrics.BuildDeltasDur
	metrics.StatsApplyDur = statsApplyMetrics.ApplyDur
	metrics.StatsItems = statsApplyMetrics.Items
	metrics.StatsNomKeys = statsApplyMetrics.NomKeys
	metrics.StatsCognomKeys = statsApplyMetrics.CognomKeys
	metrics.StatsDeltaRows = statsApplyMetrics.DeltaRows
	metrics.StatsMunicipis = statsApplyMetrics.Municipis
	metrics.StatsNivells = statsApplyMetrics.Nivells
	metrics.StatsNegativeRows = statsApplyMetrics.NegativeDeltaRows
	metrics.StatsDur = measuredDurationAtLeast(time.Since(statsStart), metrics.StatsPrepareDur, metrics.StatsAggregateDur, metrics.StatsEnsureDur, metrics.StatsBuildDeltasDur, metrics.StatsApplyDur)

	searchStart := time.Now()
	searchPrepareStart := time.Now()
	searchDocs := make([]db.SearchDoc, 0, len(successByID))
	searchDeletes := make([]int, 0, len(successByID))
	if searchCtx == nil {
		searchCtx = newModeracioBulkRegistreSearchContext()
	}
	metrics.SearchPrepareDur = time.Since(searchPrepareStart)
	buildStart := time.Now()
	cognomStats := searchCognomCacheStats{}
	docCacheHits := 0
	docCacheMisses := 0
	for _, state := range states {
		if _, ok := successSet[state.Reg.ID]; !ok {
		...
		state.Reg.ModeracioEstat = estat
		if estat == "publicat" {
			if searchCtx != nil && searchCtx.SearchDocCache != nil {
				if doc, ok := searchCtx.SearchDocCache[state.Reg.ID]; ok {
					searchDocs = append(searchDocs, doc)
					delete(searchCtx.SearchDocCache, state.Reg.ID)
					docCacheHits++
					continue
				}
			}
			docCacheMisses++
			if doc := a.buildSearchDocFromRegistreWithBulkSearchContext(&state.Reg, state.Persones, state.Llibre, state.ArxiuID, searchCtx, &cognomStats); doc != nil {
				searchDocs = append(searchDocs, *doc)
			}
	...
	metrics.SearchBuildDur = time.Since(buildStart)
	upsertStart := time.Now()
	if err := a.bulkUpsertSearchDocs(searchDocs); err != nil {
		Errorf("SearchIndex bulk upsert registre: %v", err)
	}
	metrics.SearchUpsertDur = time.Since(upsertStart)
	deleteStart := time.Now()
	if err := a.bulkDeleteSearchDocs("registre_raw", searchDeletes); err != nil {
		Errorf("SearchIndex bulk delete registre: %v", err)
	}
	metrics.SearchDeleteDur = time.Since(deleteStart)
	metrics.SearchDocsUpserts = len(searchDocs)
	metrics.SearchDocsDeletes = len(searchDeletes)
	metrics.SearchCacheHits = cognomStats.Hits
	metrics.SearchCacheMisses = cognomStats.Misses
	metrics.SearchCacheSize = len(searchCtx.CognomCanonCache)
	metrics.SearchDocCacheHits = docCacheHits
	metrics.SearchDocCacheMisses = docCacheMisses
	metrics.SearchDocCacheSize = len(searchCtx.SearchDocCache)
	if IsDebugEnabled() {
		Debugf("moderacio bulk registre search aggregate docs=%d deletes=%d cognom_canon_cache=%d doc_cache_hits=%d doc_cache_misses=%d doc_cache_remaining=%d prepare_dur=%s build_dur=%s upsert_dur=%s delete_dur=%s cache_hits=%d cache_misses=%d apply=bulk", len(searchDocs), len(searchDeletes), len(searchCtx.CognomCanonCache), metrics.SearchDocCacheHits, metrics.SearchDocCacheMisses, metrics.SearchDocCacheSize, metrics.SearchPrepareDur, metrics.SearchBuildDur, metrics.SearchUpsertDur, metrics.SearchDeleteDur, metrics.SearchCacheHits, metrics.SearchCacheMisses)
	}
	metrics.SearchDur = measuredDurationAtLeast(time.Since(searchStart), metrics.SearchPrepareDur, metrics.SearchBuildDur, metrics.SearchUpsertDur, metrics.SearchDeleteDur)
	return metrics
}
...
func (a *App) updateConfessionalEntityModerationOnly(id int, estat, motiu string, moderatorID int, before *db.EntitatReligiosa) error {
