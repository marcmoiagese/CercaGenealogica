...
type eventHistoricSnapshot struct {
	Event   db.EventHistoric         `json:"event"`
	Impacts []db.EventHistoricImpact `json:"impacts"`
}

func buildEventHistoricSnapshot(event *db.EventHistoric, impacts []db.EventHistoricImpact) (json.RawMessage, error) {
	if event == nil {
		return nil, fmt.Errorf("event nil")
	}
	payload, err := json.Marshal(eventHistoricSnapshot{
		Event:   *event,
		Impacts: impacts,
	})
	if err != nil {
...
func (a *App) canViewEventHistoric(user *db.User, perms db.PolicyPermissions, event *db.EventHistoric) bool {
	...
	if strings.TrimSpace(event.ModerationStatus) == "" || event.ModerationStatus == "publicat" {
		return true
	}
	if user == nil {
		return false
...
func (a *App) EventHistoricWikiHistory(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.NotFound(w, r)
		return
	}
	if !isValidWikiObjectType("event_historic") {
		http.NotFound(w, r)
		return
	}
	...
	*r = *a.withPermissions(r, perms)
	...
	changes, _ := a.DB.ListWikiChanges("event_historic", eventID)
	...
	if viewToken != "" {
		...
		case "current", "published":
			if len(currentSnapshot) > 0 {
				viewFields = buildWikiViewFields(currentSnapshot)
		...
		if len(parts) == 2 {
			resolveSnapshot := func(token string) (json.RawMessage, string) {
				...
				if token == "" || token == "current" || token == "published" {
					if len(currentSnapshot) > 0 {
						return currentSnapshot, T(lang, "wiki.history.current")
					}
					return nil, ""
				}
	...
	title := fmt.Sprintf("%s: %s", T(lang, "events.title"), strings.TrimSpace(event.Titol))
	historyURL := fmt.Sprintf("/historia/events/%d/historial", event.ID)
	RenderPrivateTemplate(w, r, "wiki-history.html", map[string]interface{}{
		"Title":             title,
		"BackURL":           fmt.Sprintf("/historia/events/%d", event.ID),
		"HistoryURL":        historyURL,
		"RevertURL":         fmt.Sprintf("/historia/events/%d/historial/revert", event.ID),
		"History":           history,
...
func (a *App) EventHistoricWikiStats(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.NotFound(w, r)
		return
	}
	...
	*r = *a.withPermissions(r, perms)
	...
	lang := resolveUserLang(r, user)
	counts, _ := a.DB.GetWikiPublicCounts("event_historic", eventID)
...
func (a *App) EventHistoricWikiRevert(w http.ResponseWriter, r *http.Request) {
	...
	if change.ObjectType != "event_historic" || change.ObjectID != eventID {
		http.Error(w, "Canvi invàlid", http.StatusBadRequest)
		return
	}
	...
	if newID > 0 {
		detail := "event_historic:" + strconv.Itoa(eventID)
		_, _ = a.RegisterUserActivity(r.Context(), user.ID, "", "editar", "event_historic_canvi", &newID, "pendent", nil, detail)
	}
	returnURL := strings.TrimSpace(r.FormValue("return_to"))
	if returnURL == "" {
		returnURL = fmt.Sprintf("/historia/events/%d/historial", eventID)
	}
	http.Redirect(w, r, returnURL, http.StatusSeeOther)
}

func (a *App) EventHistoricWikiMark(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.NotFound(w, r)
		return
	}
	...
	prevMark, _ := a.DB.GetWikiMark("event_historic", eventID, user.ID)
	...
	if err := a.updateWikiPublicCounts("event_historic", eventID, prevMark, tipus, isPublic); err != nil {
...
func (a *App) EventHistoricWikiUnmark(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.NotFound(w, r)
		return
	}
	...
	prevMark, _ := a.DB.GetWikiMark("event_historic", eventID, user.ID)
	if err := a.DB.DeleteWikiMark("event_historic", eventID, user.ID); err != nil {
		Errorf("Error eliminant marca wiki: %v", err)
		http.Error(w, "No s'ha pogut eliminar la marca", http.StatusInternalServerError)
		return
	}
	if prevMark != nil {
		if err := a.updateWikiPublicCounts("event_historic", eventID, prevMark, prevMark.Tipus, false); err != nil {
			Errorf("Error actualitzant stats wiki: %v", err)
			http.Error(w, "No s'ha pogut eliminar la marca", http.StatusInternalServerError)
			return
		}
...
func (a *App) applyWikiEventHistoricChange(change *db.WikiChange, motiu string, moderatorID int) error {
	_, after := parseWikiChangeMeta(change.Metadata)
	if len(after) == 0 {
		return fmt.Errorf("canvi sense dades")
	}
	var snap eventHistoricSnapshot
	if err := unmarshalWikiSnapshot(after, &snap); err != nil {
		return err
	}
	event := snap.Event
	event.ID = change.ObjectID
	event.ModerationStatus = "publicat"
	event.ModerationNotes = motiu
	event.ModeratedBy = sqlNullIntFromInt(moderatorID)
	event.ModeratedAt = sql.NullTime{Time: time.Now(), Valid: true}
	if err := a.DB.UpdateEventHistoric(&event); err != nil {
		return err
	}
	impacts := snap.Impacts
	for i := range impacts {
		impacts[i].EventID = event.ID
	}
	return a.DB.ReplaceEventImpacts(event.ID, impacts)
}
