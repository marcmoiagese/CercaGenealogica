...
func (a *App) ArxiuWikiHistory(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.NotFound(w, r)
		return
	}
	if !isValidWikiObjectType("arxiu") {
		http.NotFound(w, r)
		return
	}
	lang := ResolveLang(r)
	arxiuID := extractID(r.URL.Path)
	if arxiuID == 0 {
		http.NotFound(w, r)
		return
	}
	...
	canModerate := a.canModerateWikiObject(user, "arxiu", arxiu.ID)
	...
	changes, _ := a.DB.ListWikiChanges("arxiu", arxiuID)
	...
	if viewToken != "" {
		...
		case "current", "published":
			if payload, err := json.Marshal(arxiu); err == nil {
	...
	title := fmt.Sprintf("%s: %s", T(lang, "archives.title"), arxiu.Nom)
	historyURL := fmt.Sprintf("/documentals/arxius/%d/historial", arxiu.ID)
	RenderPrivateTemplate(w, r, "wiki-history.html", map[string]interface{}{
		"Title":             title,
		"BackURL":           fmt.Sprintf("/documentals/arxius/%d", arxiu.ID),
		"HistoryURL":        historyURL,
		"RevertURL":         fmt.Sprintf("/documentals/arxius/%d/historial/revert", arxiu.ID),
		"History":           history,
...
func (a *App) ArxiuWikiStats(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.NotFound(w, r)
		return
	}
	...
	counts, _ := a.DB.GetWikiPublicCounts("arxiu", arxiuID)
...
func (a *App) ArxiuWikiRevert(w http.ResponseWriter, r *http.Request) {
	...
	if change.ObjectType != "arxiu" || change.ObjectID != arxiuID {
		http.Error(w, "Canvi invàlid", http.StatusBadRequest)
		return
	}
	...
	if newID > 0 {
		detail := "arxiu:" + strconv.Itoa(arxiuID)
		_, _ = a.RegisterUserActivity(r.Context(), user.ID, ruleArxiuUpdate, "editar", "arxiu_canvi", &newID, "pendent", nil, detail)
	}
	returnURL := strings.TrimSpace(r.FormValue("return_to"))
	if returnURL == "" {
		returnURL = fmt.Sprintf("/documentals/arxius/%d/historial", arxiuID)
	}
	http.Redirect(w, r, returnURL, http.StatusSeeOther)
}

func (a *App) ArxiuWikiMark(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.NotFound(w, r)
		return
	}
	...
	if arxiu.ModeracioEstat != "" && arxiu.ModeracioEstat != "publicat" && !(a.CanManageArxius(user) || a.canModerateWikiObject(user, "arxiu", arxiu.ID)) {
		http.NotFound(w, r)
		return
	}
	...
	prevMark, _ := a.DB.GetWikiMark("arxiu", arxiuID, user.ID)
	...
	if err := a.updateWikiPublicCounts("arxiu", arxiuID, prevMark, tipus, isPublic); err != nil {
...
func (a *App) ArxiuWikiUnmark(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.NotFound(w, r)
		return
	}
	...
	prevMark, _ := a.DB.GetWikiMark("arxiu", arxiuID, user.ID)
	if err := a.DB.DeleteWikiMark("arxiu", arxiuID, user.ID); err != nil {
		Errorf("Error eliminant marca wiki: %v", err)
		http.Error(w, "No s'ha pogut eliminar la marca", http.StatusInternalServerError)
		return
	}
	if prevMark != nil {
		if err := a.updateWikiPublicCounts("arxiu", arxiuID, prevMark, prevMark.Tipus, false); err != nil {
			Errorf("Error actualitzant stats wiki: %v", err)
			http.Error(w, "No s'ha pogut eliminar la marca", http.StatusInternalServerError)
			return
		}
