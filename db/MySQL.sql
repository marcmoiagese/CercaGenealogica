...
CREATE TABLE IF NOT EXISTS model_confessional (
    ...
    any_inici SMALLINT,
    any_fi SMALLINT,
    estat ENUM('actiu', 'inactiu', 'historic') NOT NULL DEFAULT 'actiu',
    observacions TEXT,
    moderation_status ENUM('pendent','publicat','rebutjat') NOT NULL DEFAULT 'pendent',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS entitat_religiosa (
    ...
    pais_id INT UNSIGNED NULL,
    parent_id INT UNSIGNED NULL,
    tipus_entitat VARCHAR(100),
    tipus_especific VARCHAR(100),
    any_inici SMALLINT,
    any_fi SMALLINT,
    estat ENUM('actiu', 'inactiu', 'historic') NOT NULL DEFAULT 'actiu',
    web VARCHAR(255),
...
CREATE TABLE IF NOT EXISTS entitat_religiosa_relacio (
    ...
    entitat_desti_id INT UNSIGNED NOT NULL,
    tipus_relacio VARCHAR(100) NOT NULL,
    any_inici SMALLINT,
    any_fi SMALLINT,
...
CREATE TABLE IF NOT EXISTS arxiu_abast (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_arxiu_abast_arxiu (arxiu_id),
...
CREATE TABLE IF NOT EXISTS external_links (
  ...
  url_norm_hash CHAR(64) GENERATED ALWAYS AS (SHA2(url_norm, 256)) STORED,
  title TEXT,
  meta TEXT,
  status VARCHAR(20) NOT NULL DEFAULT 'pending',
  ...
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY idx_external_links_persona_url (persona_id, url_norm_hash),
  KEY idx_external_links_persona_status (persona_id, status),
  KEY idx_external_links_site (site_id),
  CONSTRAINT chk_external_links_status CHECK (status IN ('pending','approved','rejected')),
