...
func (d *SQLite) ReplacePoliticaGrants(politicaID int, grants []PoliticaGrant) error {
...
func (d *SQLite) SaveEntitatReligiosaRelacio(rel *EntitatReligiosaRelacio) (int, error) {
	return sqliteSaveEntitatReligiosaRelacio(d, rel)
}
func (d *SQLite) DeleteEntitatReligiosaRelacio(id int) error {
...
func (d *SQLite) GetArxiuEntitatReligiosa(id int) (*ArxiuEntitatReligiosa, error) {
	return sqliteGetArxiuEntitatReligiosa(d, id)
}
func (d *SQLite) SaveArxiuEntitatReligiosa(rel *ArxiuEntitatReligiosa) (int, error) {
	return sqliteSaveArxiuEntitatReligiosa(d, rel)
}
func (d *SQLite) DeleteArxiuEntitatReligiosa(id int) error {
	return sqliteDeleteArxiuEntitatReligiosa(d, id)
}
...
func (d *SQLite) BulkInsertLlibres(ctx context.Context, rows []Llibre) ([]int, string, error) {
	if len(rows) == 0 {
		return nil, "sqlite-tx", nil
	}
	tx, err := d.Conn.BeginTx(ctx, nil)
	...
	for i := 0; i < len(rows); i += bulkLlibresBatchSize {
		...
		lastID, err := res.LastInsertId()
		if err != nil || lastID <= 0 {
			return nil, "sqlite-tx", fmt.Errorf("sqlite bulk insert llibres: no last insert id")
		}
		firstID := lastID - int64(len(batch)-1)
		for j := 0; j < len(batch); j++ {
			ids = append(ids, int(firstID)+j)
...
func (d *SQLite) BulkInsertArxiuLlibres(ctx context.Context, rows []ArxiuLlibreLink) (string, error) {
	if len(rows) == 0 {
		return "sqlite-tx", nil
	}
	tx, err := d.Conn.BeginTx(ctx, nil)
...
func (d *SQLite) BulkInsertLlibreURLs(ctx context.Context, rows []LlibreURL) (string, error) {
	if len(rows) == 0 {
		return "sqlite-tx", nil
	}
