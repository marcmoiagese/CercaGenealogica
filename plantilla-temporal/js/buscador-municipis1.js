document.addEventListener('DOMContentLoaded', function () {
    ...
    const dadesLocalitzacio = {
        espanya: {
            ...
            comunitats: {
                ...
                asturias: {
                    nom: "Asturias",
                    columnes: ["Pais", "Comunitat Autonoma", "Provincia", "Comarca", "Concejo", "Registres", "Opcions"],
                    provincies: {
                        asturies: {
                            nom: "Asturies",
                            comarques: [
                ...
                cantabria: {
                    nom: "Cantabria",
                    columnes: ["Pais", "Comunitat Autonoma", "Provincia", "Vall", "Poble", "Registres", "Opcions"],
                    provincies: {
                        cantabria: {
                            nom: "Cantàbria",
                            valls: [
    ...
    function generarDadesMunicipis() {
        ...
        Object.values(dadesLocalitzacio.espanya.comunitats).forEach(comunitat => {
            ...
            Object.entries(comunitat.provincies).forEach(([provKey, provincia]) => {
                if (!provincia.mancomunitats) return;
                provincia.mancomunitats.forEach(mancomunitat => {
                    if (!Array.isArray(mancomunitat.municipis)) return;
                    mancomunitat.municipis.forEach(poble => {
                        const registres = Math.floor(Math.random() * 100);
                        const indexat = Math.min(100, Math.round((Math.random() * 100) + 30)); // Ex: entre 30 i 100%
                        municipis.push({
                            pais: "Espanya",
                            comunitat: comunitat.nom,
                            provincia: provincia.nom,
                            mancomunitat: mancomunitat.nom,
                            poble: poble,
                            registres: registres,
                            indexat: indexat
                        });
    ...
    function mostrarProvincies() {
        ...
        const comunitat = dadesLocalitzacio.espanya.comunitats[comunitatId];
    ...
    function mostrarMancomunitats() {
        ...
        const provinciaKey = Object.keys(comunitat.provincies).find(key =>
            normalitzar(key) === normalitzar(provinciaId)
        );

        const provincia = provinciaKey ? comunitat.provincies[provinciaKey] : null;

        if (!provincia || !provincia.mancomunitats) {
            mancomunitatGrup.style.display = 'none';
            municipiGrup.style.display = 'none';
            mancomunitatSelect.innerHTML = '<option value="">-- Selecciona opció --</option>';
            municipiSelect.innerHTML = '<option value="">-- Selecciona municipi --</option>';
        ...
        omplirSelect(mancomunitatSelect, provincia.mancomunitats.map(mc => ({ nom: mc.nom })));
    ...
    function mostrarMunicipis() {
        ...
        const provincia = provinciaKey ? comunitat.provincies[provinciaKey] : null;
        const mancomunitatData = provincia?.mancomunitats?.find(m =>
